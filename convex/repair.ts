...
async function upsertSceneFull(ctx: any, slug: string, title: string, domain: string, bgUrl: string, objects: any[]) {
    ...
    if (scene) {
        sceneId = scene._id;
        // Check if bgUrl is provided, otherwise keep existing
        if (bgUrl) {
        ...
        sceneId = await ctx.db.insert("scenes", {
            ...
            backgroundMediaUrl: bgUrl || "", // Fallback
            isPublished: true,
        });
    }

    // 2. Objects & Reveals (Only if they don't exist? For repair, let's just add if scene had 0 objects)
    const existingObjects = await ctx.db.query("objects").withIndex("by_scene", (q: any) => q.eq("sceneId", sceneId)).collect();
...
export const repairScenes = mutation({
    args: {},
    handler: async (ctx) => {
        // Repair Study
        ...
        // Repair Workshop (Just to be safe, though likely exists)
        await upsertSceneFull(
            ctx,
            "workshop",
            "The Workbench",
            ...
            [] // Objects likely exist
        );

        return "Repaired Scenes";
    }
});
